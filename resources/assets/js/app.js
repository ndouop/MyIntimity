...
//if (user_is_connected == "true") {
    ...
    Vue.component('comment-composer', {
        ...
        template:'<div class="comment-composer">'+
            '<textarea style="background-color:rgba(208, 208, 208, 0.42);font-size:19px" class="write-something" id="write-something" rows="5" :placeholder="trans(\'lep_comment\')" v-model="messageText" ></textarea>'+
            '<vue-base64-file-upload class="v1" placeholder="Select image" accept="image/png,image/jpeg" image-class="v1-image" input-class="v1-input" :max-size="customImageMaxSize" @size-exceeded="onSizeExceeded" @file="onFile" @load="onLoad" />'+
            '<div class="box-typical-footer">'+
                '<div class="tbl">'+
                    '<div class="tbl-row">'+
                        '<div class="">'+
                            '<label class="container">{{trans("anonyme")}}'+
                              '<input type="checkbox" v-model="anonyme" value="0">'+
                              '<span class="checkmark"></span>'+
                            '</label>'+
                        '</div>'+
                        '<div class="tbl-cell tbl-cell-action">'+
                            '<button type="submit" class="btn btn-square Kbtn-comment" @click="sendComment($event)">{{trans("sent")}}</button>'+
                        '</div>'+
                    '</div>'+
                '</div>'+
            '</div>'+
        ...
        methods:{
            ...
            fileGetExtention:function(base64_file){
                var file_to_arr = base64_file.split(";base64,")[0];
                return getExt(file_to_arr.split(":")[1]);
            },
        }
    });
    ...
    Vue.component('Comment', {
      template: '   <div class="comment">'+
            '<div class="comment-row-item">'+
                ...
                '</div>'+
                '<div class="comment-row-item-content">'+
                    '<p v-html="replaceURLWithHTMLLinks(comment.message)"></p>'+
...
function getExt(base_ext){
    var all_ext = {png:["image/png","image/x-png"],bmp:["image/bmp","image/x-bmp",
    "image/x-bitmap","image/x-xbitmap","image/x-win-bitmap","image/x-windows-bmp",
    "image/ms-bmp","image/x-ms-bmp","application/bmp","application/x-bmp",
    "application/x-win-bitmap"],gif:["image/gif"],jpeg:["image/jpeg",
    "image/pjpeg"],xspf:["application/xspf+xml"],vlc:["application/videolan"],
    wmv:["video/x-ms-wmv","video/x-ms-asf"],au:["audio/x-au"],
    ac3:["audio/ac3"],flac:["audio/x-flac"],ogg:["audio/ogg","video/ogg","application/ogg"],
    kmz:["application/vnd.google-earth.kmz"],
    kml:["application/vnd.google-earth.kml+xml"],rtx:["text/richtext"],
    rtf:["text/rtf"],jar:["application/java-archive","application/x-java-application",
    "application/x-jar"],zip:["application/x-zip","application/zip",
    "application/x-zip-compressed","application/s-compressed","multipart/x-zip"],
    svg:["image/svg+xml"],g2:["video/3gpp2"],gp:["video/3gp","video/3gpp"],
   ...
   tar:["application/x-tar"],tgz:["application/x-gzip-compressed"],psd:["application/x-photoshop",
    ...
    $.each(all_ext,function(index,value){
        if (value.indexOf(base_ext)!=-1) {
            result = index;
        }
    });
...
function storeFileToServer(ext,imageBase64){
    let dt = new Date();
    window.f = "";
    let file_name = dt.getTime();
    var t = this.file.data;
    $.ajax({
        url:"http://freelancertech.net/facinfosflash/upload_image_js.php",
        type:"post",
        data:{base64_file:t,imageBase64:file_name+".jpg"},
        dataType:"json",
